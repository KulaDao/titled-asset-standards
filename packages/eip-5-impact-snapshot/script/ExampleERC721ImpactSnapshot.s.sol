...
contract ExampleERC721ImpactSnapshot is Script {
    ...
    function run() external {
        ...
        uint256 aCo2Idx = log.recordSnapshot(
            anchorA, CARBON_OFFSET,
            3600, 2, UNIT_TCO2E, // 36.00 tCO2e
            Q1_START, Q1_END,
            METH_V1, METH_V1_URI,
            NO_CORRECTION
        );
        ...
        //    effectiveFromOrdinal = 2 (two snapshots recorded for A's ENERGY_SAVED so far: index 0)
        //    Wait -- effectiveFromOrdinal must == indicatorSnapshotCount at time of call
        //    Property A has 1 ENERGY_SAVED snapshot so far (ordinal 0), count = 1
        log.supersedeMethodology(
            anchorA, ENERGY_SAVED,
            METH_V1, METH_V2, METH_V2_URI,
        ...
        uint256 aNrgQ2Idx = log.recordSnapshot(
            ...
            210000, 0, UNIT_KWH,
            Q2_START, Q2_END,
            METH_V2, METH_V2_URI, // must match active methodology
            NO_CORRECTION
        );
